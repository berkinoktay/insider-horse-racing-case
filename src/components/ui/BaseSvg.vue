<template>
  <span ref="icon" class="flex items-center justify-center max-w-max max-h-max" :style="style" v-html="svgEl"></span>
</template>

<script setup lang="ts">
import { onMounted, onServerPrefetch, ref, useTemplateRef, watch, type CSSProperties } from 'vue'
...
async function loadSvg() {
  try {
    if (props.src) {
      ...
      const module = await import(`@/assets/svg/${props.name}.svg?raw`).then((m) => m.default)
...
function parseSvg(svgData: string) {
  ...
  svgData = svgData.replace(/<svg\b([^>]*)>/, (_, attributes) => {
    return props.preserveColor
      ? `<svg ${attributes} width="${props.size}" height="${props.size}">`
      : `<svg ${attributes} width="${props.size}" height="${props.size}" fill="currentColor">`
  })
